...
public class Missile : Obstacle {
    ...
    public void Update()
    {
        if (TrackManager.instance == null || TrackManager.instance.segments == null)
        {
            return;
        }
        ...
        {
            if (TrackManager.instance.segments.Count <= 1)
            {
                return;
            }

            if (m_OwnSegement != null && TrackManager.instance.segments[1] == m_OwnSegement)
            {
                if(animator != null)
                {
